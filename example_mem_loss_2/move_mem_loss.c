...
#define SSTEP     600        // numero de pasos totales a intervalos de SAMPLE
#define WAIT      20         // 
...
/*
 * move_mem_loss.c (make the simulation assuming memoryless pedestrians)
 *
 * gcc -O3 -Wall -O3 -o move_mem_loss.e move_mem_loss.c -lm
 * 
 * Autor: Guillermo Frank   19 febrero 2020
 *
 * ./move_mem_loss.e myfile.dat myfile_rule.lammpstrj
 */
...
int     desire_test(int *lss,int *mem,int *ti,double *xp,double *tt,double *xb,double *x,double *v0,int *w,int *nm,int np,int n,int nb);
int     update(int *mem,double *tt,double *x,double *xb,int *nm,int np,int n,int nb);
int     reset(int *mem,double *tt,double *x,double *xb,int *nm,int n,int nb);
...
int     targets(int *del,int *mem,int *ti,double *tp,double *tt,double *x,double *xb,int *w,int *nm,int np,int n,int nb);
...
int main(int argc, char *argv[])
{
  ...
  int     i,n,nb,np,nc,*w,*head,*lscl,*ti,*mem,*nm,*lss;
  double  t,scale;
  ...
  targets(lss,mem,ti,tp,tt,x,xb,w,nm,np,n,nb);

  desire_test(lss,mem,ti,xp,tt,xb,x,v0,w,nm,np,n,nb);
   ...
   for(i=0;i<n;i++)
     {
       ...
       update(mem,tt,x,xb,nm,np,n,nb);

       h = neighbor(ti,xp,p1,np);                          // get closest invisible target

       tnew = *(mem+n*i+h-1);

       // at start-up (told=0) the pedestrian decides the 
       // move to the closest invisible target 

       if ((told==0) && (tnew>0))
         {
           *(lss+n*i) = tnew;
           move2target(v0,tt,x,xb,i,tnew,np,nb);
           ...
           printf("pedestrian %d: ",i);
           printf("\ttarget: %d --> %d (%lf,%lf) ",told,tnew,tx,ty);
           printf("\tmem(%d) = ",i);
           for (j=0;j<(*(nm+i));j++) 
             {
               if ((*(mem+n*i+j))<0) printf("%d ",*(mem+n*i+j));
               else printf("+%d ",*(mem+n*i+j));
             }
           printf("\n");
       ...
       // the pedestrian is already on his (her) way (told!=0), but a new 
       // invisible target appears (told!=tnew). The pedestrian changes his (her) mind 
       // only if he (she) fulfills the previous objective, that is, getting 
       // to the former target until it becomes visible ((*(mem+n*i+told-1))<0).  
       // In order to make a new decision, a completely new examination of visible
       // targets is performed (reset function).
       // 
       ...
       if ((told!=0) && (tnew<0) && ((*(mem+n*i+told-1))<0)) (*(w+i))++;
...
int targets(int *lss,int *mem,int *ti,double *tp,double *tt,double *x,double *xb,int *w,int *nm,int np,int n,int nb)
{
  int    h,i,j,nt;
  double xi,yi,*p1,*p2;
  ...
  for(i=0;i<n;i++) 
    {
      *(w+i) = 0;   // the waiting number of events due to patience.
      *(lss+i) = 0; // the remembered targets are set to null.
    }
  ...
  for(h=0;h<n;h++)
    {
      ...
      for(i=0;i<nt;i++)
        {        
          ...
          if(!intersection(xb,p1,p2,nb)) *(mem+h*n+i) = -(i+1); // set to visible 
        }
    }
